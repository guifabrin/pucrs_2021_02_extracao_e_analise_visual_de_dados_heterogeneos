...
import libs.snscrape.base

logger = logging.getLogger(__name__)


class Tweet(libs.snscrape.base.Item):
...
class Account(libs.snscrape.base.Item):
...
class TwitterCommonScraper(libs.snscrape.base.Scraper):
...
class TwitterSearchScraper(TwitterCommonScraper):
    ...
    def _get_guest_token(self):
        ...
        if 'gt' in r.cookies:
            ...
            return r.cookies['gt']
        raise libs.snscrape.base.ScraperException('Unable to find guest token')
    ...
    def get_items(self):
        ...
        while True:
            ...
            try:
                ...
                raise libs.snscrape.base.ScraperException('Received invalid JSON from Twitter') from e
                ...
                for entry in entries:
                    if entry['entryId'].startswith('sq-I-t-'):
                        if 'tweet' in entry['content']['item']['content']:
                            ...
                            raise libs.snscrape.base.ScraperException(f'Unable to handle entry {entry["entryId"]!r}')
                        tweetID = tweet['id']
...
class TwitterListMembersScraper(TwitterCommonScraper):
    ...
    def get_items(self):
        ...
        if not container:
            raise libs.snscrape.base.ScraperException('Unable to find container')
