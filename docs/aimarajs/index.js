...
function createTree(p_div, p_backColor, p_contextMenu) {
  const tree = {
    ...
    createNode: function(p_text, p_expanded, p_icon, p_parentNode, p_tag, p_contextmenu) {
      ...
      if (this.rendered) {
        if (p_parentNode==undefined) {
          ...
          const v_ul = p_parentNode.elementLi.getElementsByTagName('ul')[0];
    ...
    drawTree: function() {
      ...
      const div_tree = document.getElementById(this.div);
      div_tree.innerHTML = '';
    ...
    drawNode: function(p_ulElement, p_node) {
      ...
      const v_li = document.createElement('li');
      p_node.elementLi = v_li;

      const v_span = createSimpleElement('span', null, 'node');
      ...
      const v_ul = createSimpleElement('ul', 'ul_' + p_node.id, null);
    ...
    expandNode: function(p_node) {
      if (p_node.childNodes.length>0 && p_node.expanded==false) {
        ...
        const img=p_node.elementLi.getElementsByTagName('img')[0];
    ...
    selectNode: function(p_node) {
      const span = p_node.elementLi.getElementsByTagName('span')[0];
    ...
    removeNode: function(p_node) {
      const index = p_node.parent.childNodes.indexOf(p_node);
      ...
      if (p_node.parent.childNodes.length==0) {
        const v_img = p_node.parent.elementLi.getElementsByTagName('img')[0];
    ...
    removeChildNodes: function(p_node) {
      if (p_node.childNodes.length>0) {
        const v_ul = p_node.elementLi.getElementsByTagName('ul')[0];

        const v_img = p_node.elementLi.getElementsByTagName('img')[0];
        v_img.style.visibility = 'hidden';
    ...
    nodeContextMenu: function(p_event, p_node) {
      if (p_event.button==2) {
        ...
        if (p_node.contextMenu!=undefined) {
          ...
          const v_menu = this.contextMenu[p_node.contextMenu];
          ...
          const v_left = p_event.pageX-5;
          const v_right = p_event.pageY-5;
          ...
          for (let i=0; i<v_menu.elements.length; i++) {
            (function(i) {
              const v_li = createSimpleElement('li', null, null);

              const v_span = createSimpleElement('span', null, null);
              v_span.onclick = function() {
                v_menu.elements[i].action(p_node);
              };

              const v_a = createSimpleElement('a', null, null);
              const v_ul = createSimpleElement('ul', null, 'sub-menu');
              ...
              if (v_menu.elements[i].icon!=undefined) {
                const v_img = createImgElement('null', 'null', v_menu.elements[i].icon);
              ...
              if (v_menu.elements[i].submenu!=undefined) {
                const v_span_more = createSimpleElement('div', null, null);
    ...
    contextMenuLi: function(p_submenu, p_ul, p_node) {
      ...
      for (let i=0; i<p_submenu.elements.length; i++) {
        (function(i) {
          const v_li = createSimpleElement('li', null, null);
          ...
          if (p_submenu.elements[i].icon!=undefined) {
            const v_img = createImgElement('null', 'null', p_submenu.elements[i].icon);
            v_li.appendChild(v_img);
          }
    ...
    adjustLines: function(p_ul, p_recursive) {
      ...
      if (tree.childNodes.length>0) {
        ...
        if (p_recursive) {
            ...
            const check_ul = tree.getElementsByTagName('ul')[i];
